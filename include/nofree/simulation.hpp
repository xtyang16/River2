...
#ifndef RIVER2_SIMULATION_HPP
#define RIVER2_SIMULATION_HPP
...
namespace ns_river2
{
  ...
  public:
    explicit Simulator(std::string savePath, CalibParamManager::Ptr calibParamManager,
        ...
        : _savePath(std::move(savePath)), _parMagr(std::move(calibParamManager)),
          _traj(ns_ctraj::RiverMotion<4>(1.0, 50.0, staDur, motionDur, 5)), _gravity(0.0, 0.0, -9.8) {}
                  ...
                  double acceBias, double gyroBias)
    {
      ...
      int _tab = 0;
      for (const auto &[imuTopic, _] : _parMagr->EXTRI.POS_BiInBi0)
      {
        SimulateIMU(bag, imuTopic, imuHZ, acceNoise, gyroNoise, acceBias, gyroBias, _tab++);
      }
      _tab = 0;
      for (const auto &[radarTopic, _] : _parMagr->EXTRI.POS_RjInBi0)
      {
        SimulateRadar(bag, radarTopic, radarHZ, radarNoise, _tab++);
      }
      bag->close();
    }
  ...
  protected:
    void SimulateIMU(const std::unique_ptr<rosbag::Bag> &bag, const std::string &imuTopic, int hz,
                     double acceNoise, double gyroNoise, double acceBias, double gyroBias, int tab)
    {
      // imu data
      auto imuMes = this->_traj.GetTrajectory()->ComputeIMUMeasurement(_gravity, _parMagr->EXTRI.SE3_BiToBc(imuTopic), 1.0 / hz);
      ...
      {
        if (frame->GetTimestamp() < ros::TIME_MIN.toSec())
        {
          continue;
        }

        sensor_msgs::Imu imuMsg;
        imuMsg.header.stamp = ros::Time(frame->GetTimestamp() - _parMagr->TEMPORAL.TIME_OFFSET_BiToBi0.at(imuTopic));
        imuMsg.header.frame_id = "imu_" + std::to_string(tab);
                       ...
                       double radarNoise, int tab)
    {
      ...
      for (const auto &item : _traj.GetTrajectory()->Sampling(1.0 / static_cast<double>(hz)))
      {
        ...
        auto obv = RadarTarget::Create(
            timeByIMU - _parMagr->TEMPORAL.TIME_OFFSET_RjToBi0.at(radarTopic),
            // attention: here is 'timeByIMU', not 'timeByIMU - TIME_OFFSET_RtoB'
            _traj.GetTrajectory()->RadarStaticMeasurement(
                timeByIMU, targets.at(uTarIdx(engine)), _parMagr->EXTRI.SE3_RjToBc(radarTopic)) +
      ...
      {
        if (tar->GetTimestamp() < ros::TIME_MIN.toSec())
        {
          continue;
        }
        ...
        tarMsg.header.frame_id = "radar_" + std::to_string(tab);
