...
namespace ns_river2
{
  ...
  void Viewer::RunViewer()
  {
    ...
    spdlog::info("'Viewer::RunViewer' has been booted, thread id: {}.", RIVER2_TO_STR(std::this_thread::get_id()));
    ...
    while (ros::ok() && this->IsActive())
    {
      ...
      {
        spdlog::warn("'Viewer::RunViewer' quits normally by status.");
        break;
      }
  ...
  void Viewer::UpdateSplineWindow(const RiverStatus::StatusPack &status)
  {
    ...
    for (const auto &pack : statePacks)
    {
      ...
      if (configor->preference.VisualizeVelocityInBody)
      {
        ...
        pose = ns_viewer::Posed(pack->SO3_Bi0ToRef.matrix(), pack->LIN_VEL_Bi0ToRefInCur()).cast<float>();
      }
      else
      {
        pose = ns_viewer::Posed(pack->SO3_Bi0ToRef.matrix(), pack->LIN_VEL_Bi0ToRefInRef).cast<float>();
      }
  ...
  void Viewer::UpdateVRWindow(const RiverStatus::StatusPack &status)
  {
    ...
    auto newState = stateMagr->GetStatePackSafely(
        status.ValidStateEndTime - Configor::Preference::StatePublishDelay);
    const auto &body = newState->SO3_Bi0ToRef;
    ...
    Eigen::Vector3d ANG_VEL_CurToWinW, LIN_VEL_CurToWinW;
    {
      ...
      ANG_VEL_CurToWinW = newState->SO3_Bi0ToRef * so3Spline.VelocityBody(
                                                       status.ValidStateEndTime - Configor::Preference::StatePublishDelay);
    ...
    auto SO3_WtoCur = newState->SO3_Bi0ToRef.inverse();
    for (const auto &[p, c] : virMarkers)
    {
      Eigen::Vector3d pInBody = SO3_WtoCur * p;
      Eigen::Vector3d v = Sophus::SO3d::hat(newState->SO3_Bi0ToRef * pInBody) * ANG_VEL_CurToWinW;
